...
function beginFlow(ctx, typeText) {
  const chatId = ctx.chat.id  // ...existing code...
    ctx.reply(
      `<b>Привет!  
  Добро пожаловать в Coffeebox Uzbekistan —  
  сеть автоматических кофеен без бариста, без аренды, с телеметрией и поддержкой.  
  
  Мы — лидер рынка в Казахстане: почти 300 кофеен уже работают и приносят прибыль партнёрам.  
  Теперь Coffeebox заходит в Узбекистан 🇺🇿.
